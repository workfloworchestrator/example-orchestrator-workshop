...
def test_create_nodes(products):
    logger.debug("Running test_create_node ")
    node_names = ["loc1-core", "loc2-core", "loc3-core", "loc4-core", "loc5-core"]
    for node in node_names:
        logger.debug("Attempting to enroll node", node=node)
        node_id = create_node(products, node)
        logger.debug("Node {node} sucessfully enrolled with subscription ID: {node_id}")
        ...
        logger.debug("All assertions pass for Node Subscription", node_id=node_id)


def test_create_circuit(products):
    logger.debug("Running test_create_circuit ")
    circuit_names = [
    ...
    for circuit in circuit_names:
        logger.debug("Attempting to provision circuit", circuit=circuit)
        circuit_sub_id = create_circuit(products, circuit)
        logger.debug(
            "Circuit subscription sucessfully completed.",
            circuit=circuit,
            subscription_id=circuit_sub_id,
        )
        ...
        assert (
            circuit_model["circuit"]["members"][0]["port"]["port_name"]
            == parsed_circuit[3]
        )
        assert (
        ...
        assert (
            circuit_model["circuit"]["members"][1]["port"]["port_name"]
            == parsed_circuit[5]
        )
        assert (
